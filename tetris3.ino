#include <LedControl.h> //we include the library that we previously installed
 
LedControl lc = LedControl(12, 11, 10, 2); // this function relates the phisical pins from the arduino to functions in our LED matrix(dataPin, clockPin, csPin, totalDevices nº of matrix)
 
int lc0[] = {0, 0, 0, 0, 0, 0, 0, 0}; //here we start all our variables in the code
...
int rotate_button = 2; //all of these are the functions of the buttons we'll use to play the game
int left_button   = 3;
int down_button   = 4;
int right_button  = 5;
int start_button  = 6;
int speaker_pin   = 7;
int sound_button  = 8; //this one is not a button
...
int tempo = 128;        // this will set the tempo for our buzzer
...
void playNote(char note, int duration) { //this function controls wich note is being played and its udration
  char names[] = { 'c', 'd', 'e', 'f', 'g', 'a', 'b', 'C' , 'D', 'E', 'F', 'G', 'J', 'A', 'B'}; //we use letters to combine tones and making it easier to write all the melodies
  int tones[] = { 1915, 1700, 1519, 1432, 1275, 1136, 1014, 956, 850, 760, 716, 637, 603, 568, 507 };
 
  for (int i = 0; i < 14; i++) {
    if (names[i] == note) { //Here we assign each letter whit its corresponding tone
...
void buttonDelay(int bdelay) //here we put delay to the buttons else it would be hard to control them and the number of clicks.
{
...
void splashScreen() //line for line we translate letters to points in our display. 1 is LED on, 2 is LED off
{
     int up[] =
     {
         B11101010,   //   o o o   o   o
...
void setup() { //here starts the setup
  pinMode(speaker_pin,  OUTPUT); //music starts playing
  pinMode(rotate_button,INPUT_PULLUP); //all the buttons are on now so they are susceptible to touch
...
void loop()
{
 ...
 else
 {
     ...
     while(started == 1) //this part of the program controls the game you see
     {
 
         if(allLines < 100) //each level equals a certain speed  of the blocks that fall so it gets harder everytime
         {
             level = 0;              //Level 0
         }
         ...
         if(figura == 0)
         {
              figura = random(1,8); //as the original game ours has 8 different bricks
         }
         else
         {
             figura = figuraNext;
         ...
         switch(figura) //we have 8 diferent cases of falling bricks
         {
             case 1:
             //"O"
         ...
         for(int krok = 0; krok < lcRows + 1; krok++)
         {
             ...
             {
                 if((digitalRead(left_button) == LOW) && (fromLeft > 0)) //here we realte the input from the buttons to the bricks in the game
                 {
                     moveCheck = 0;
             ...
             {
                 brickDelay = 0;
                 playNote('b', 10); //every move has its own sound so the game feels more responsive
             }
             else
             {
                 brickDelay = defDelay;
             ...
             for(int i = 0; i < (lcCols / 2); i++)
             {
                 if(digitalRead(rotate_button) == LOW) //this function controls the rotation of the bricks while they are falling
                 {
 
                     switch(figura)
                     {
                         case 1: //this function is hard to program and the way to do it is brick by brick and by its 4 different possible rotations
                         //"O"
                             break;
             ...
             if(digitalRead(start_button) == LOW) //this function restarts the game once you press it
             {
                 memset(lc0, 0, sizeof(lc0));
                 memset(lc1, 0, sizeof(lc1));
                 memset(active, 0, sizeof(active));
...
void gameOver()
{
  ...
  playNote('A', 60);
     ...
     int cima[] =
     {
         B11111111,   //all the first matrix is turned on
         B11111111,  
         B11111111,   
         B11111111,   
         B11111111,   
     ...
     int baixo[] =
         ...
         B11111111,   //all the second matrix gets on
         B11111111,     
         B11111111,    
         B11111111,  
         B11111111,     
